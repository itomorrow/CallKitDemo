...
#define LogCError(frmt, ...)    LogObjc(LOG_FLAG_ERROR,   (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)
#define LogCWarn(frmt, ...)     LogObjc(LOG_FLAG_WARN,    (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)
#define LogCInfo(frmt, ...)     LogObjc(LOG_FLAG_INFO,    (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)
#define LogCVerbose(frmt, ...)  LogObjc(LOG_FLAG_VERBOSE, (@"%@: " frmt), THIS_FILE, ##__VA_ARGS__)

#define LogTrace()              LogObjc(LOG_FLAG_VERBOSE, @"%@: %@", THIS_FILE, THIS_METHOD)
#define LogCTrace()             LogObjc(LOG_FLAG_VERBOSE, @"%@: %s", THIS_FILE, __FUNCTION__)
...
//  read/write limitation
    ...
    unsigned long long totalWriteBytes;
    unsigned long long totalReadBytes;
    
    NSInteger          readLimitation;
    NSInteger          writeLimitation;
}
...
- (void) setLimitation:(NSInteger)readBPS writeLimiation:(NSInteger)writeBPS {
    
    dispatch_async(socketQueue, ^{
        
        startTimeInterval = [[NSDate date] timeIntervalSince1970] * 1000;
        ...
        readLimitation = readBPS;
        writeLimitation = writeBPS;
    }) ;
}
...
- (void)completeCurrentWrite
{
	...
	if (delegateQueue && [theDelegate respondsToSelector:@selector(socket:didWriteDataWithTag:)])
	{
        ...
        if (isSpeedLimited) {
            totalWriteBytes += currentWrite->bytesDone;
            double duration = [[NSDate date] timeIntervalSince1970]*1000 - startTimeInterval;
            double bps = (double)totalWriteBytes/duration*1000;
            
            if (writeLimitation > 0 && bps > writeLimitation) {
                double sleepS = MIN((double)(bps-writeLimitation)/writeLimitation, 1.f);
                
                [self suspendWriteSource] ;
                // dispatch_suspend(socketQueue) ;
                //HMLogDebug(LogModuleEStorage, @"%@ ----------- tag: %d suspendWriteSource --", self, (int)theWriteTag);
                
                __weak GCDAsyncSocket *weakSelf = self;
                
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(sleepS * NSEC_PER_SEC)), socketQueue, ^{
                    //HMLogDebug(LogModuleEStorage, @"%@ ----------- tag: %d resumeWriteSource", self, (int)theWriteTag);
                    // dispatch_resume(socketQueue) ;
                    
                    __strong GCDAsyncSocket *strongSelf = weakSelf;
                    if (strongSelf == nil)  return ;
                    
                    [strongSelf resumeWriteSource] ;
                    //HMLogDebug(LogModuleEStorage, @"%@ ----------- tag: %d resumeWriteSource end", self, (int)theWriteTag);
